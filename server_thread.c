...
//handles the connection request (cfd) via threading
void *connection(void *cfd){
	...
	//our response.
	int connfd = *(int*)cfd;
	//free malloced pointer from main()
	free(cfd);
	...
	else
	{
		...
		send(connfd, response, strlen(response), 0);
		int fileLength = file_stats.st_size;
		...
		diff = (endTime.tv_sec - startTime.tv_sec) + (endTime.tv_nsec - startTime.tv_nsec)
			/ 1000000000.0;
		//lock mutex
		pthread_mutex_lock(&mutex);
		//write to stats_thread.txt
		FILE *file = fopen("stats_thread.txt", "a");
		if (file == NULL){
			perror("stats_thread.txt error");
		} else {
			fprintf(file, "%s\t%d\t%.4f\t%.4f\n", filename, fileLength,
				endTime.tv_sec + endTime.tv_nsec / 1000000000.0, diff);
		}
		fclose(file);
		//unlock mutex
...
int main()
{
	...
	for(;;)
	{
		...
		//alloacte space for pointer to send into thread function (free in function)
		int *cfd = malloc(sizeof(int));
		*cfd = connfd;
		//handle connection with thread
		if(pthread_create(&t, NULL, connection, cfd) < 0){
			perror("pthread_create failed");
		}
		//connfd is closed in each thread individually
