//author: Griffin McCool + provided code from server.c
#include <sys/types.h>
...
int main()
{
	...
	float diff;
	int fileLength, connfd;
	//allocate space for the semaphore to make sure that all processes have acess to it
	semaphore *sem = mmap(NULL, sizeof(semaphore), PROT_READ | PROT_WRITE, MAP_SHARED |
				MAP_ANONYMOUS, -1, 0);
	sem_init(&sem->s, 1, 1);
	...
	for(;;)
	{
		...
		//enter if child (pid = 0)
		if ((pid = fork()) == 0){
			clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &startTime);
			//child doesn't need sfd copy, so we close it
			close(sfd);
			...
			else
			{
				...
				diff = (endTime.tv_sec - startTime.tv_sec) + (endTime.tv_nsec -
					startTime.tv_nsec) / 1000000000.0;
				//lock sem->s because the file is a shared resource
				sem_wait(&sem->s);
				//write to stats_proc.txt
				FILE *file = fopen("stats_proc.txt", "a");
				if (file == NULL){
					perror("stats_proc.txt error");
				} else {
					fprintf(file, "%s\t%d\t%.4f\t%.4f\n", filename,
						fileLength, endTime.tv_sec + endTime.tv_nsec
						/ 1000000000.0, diff);
				}
				fclose(file);
				//unlock
			...
			close(connfd);
			//exit child process as it is no longer needed
			exit(0);
		} else if (pid < 0){
			perror("Fork failed");
		}
		//close connection in parent process (not needed because child
		//handles the connection)
		close(connfd);

	}
	//destroy the semaphore and close sfd
	sem_destroy(&sem->s);
